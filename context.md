...
## Vercel Stateless Polling Refactor (2026-08-21)

- Removed the long-lived SSE implementation. `/api/stream` now returns `410 Gone`; the browser polls `GET /api/transactions/latest?since=<timestamp>` every 1.75 seconds, prevents overlapping calls, tracks the returned cursor, and merges each response into the dashboard state.
- `lib/durable-run-store.ts` stores the active run in Vercel KV using its REST API. The record includes the run ID, durable start time, source kind, and uploaded rows. Production returns a configuration error without KV instead of silently relying on per-invocation memory. Local development uses the ignored `.fraudlens-local-run.json` fallback only.
- `lib/orchestrator.ts` no longer starts intervals or background work. A request deterministically replays the finite source to the position implied by the durable start timestamp, then returns the current snapshot plus only transaction/agent updates newer than the supplied cursor. This makes cold starts and cross-instance invocations produce the same simulation state while keeping the polling handler fast.
- Verified locally that a start response and a later polling response shared the same run ID, and that a later cursor returned one new transaction rather than a held-open stream. `npm run typecheck`, `npm run lint`, and `npm run build` all passed.
